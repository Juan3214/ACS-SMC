...
class stats():
    ...
    def evaluatePoissonPrior(self, a, x):
    ...
    def weighted_voting(self,trees,all_labels_from_all_trees,weight,alpha_poisson=1):
        weight_per_tree = [] 
        for tree in trees: 
            if weight ==0:
                # uses accuracy of the test for the weights
                labels=stats(tree,tree.X_train).predict_for_one_tree(tree,tree.X_train)
                acc = accuracy(labels,tree.y_train) 
                weight_per_tree.append(np.exp(acc))
            if weight == 1:
                # uses evaluation function of the tree for the accuracy
                target,leaf_prob = calculate_leaf_occurences(tree)
                weight_per_tree.append(np.exp(target))
            if weight == 2:
                # uses evaluation function of the tree for the accuracy
                target,leaf_prob = calculate_leaf_occurences(tree)
                target = target + self.evaluatePoissonPrior(alpha_poisson,tree)
                weight_per_tree.append(np.exp(target))

        sum_data = []
        for i in range(0,len(all_labels_from_all_trees[0])):
            sum_data.append({})
        for i,data_tree in enumerate(all_labels_from_all_trees): 
            for j,data in enumerate(data_tree):   
                if data in sum_data[j]:
                    sum_data[j][data]+= weight_per_tree[i]
                else:
                    sum_data[j][data] = weight_per_tree[i]
        prediction = []
        for i in range(len(sum_data)):
            label=max(sum_data[i], key=sum_data[i].get)
...
def improved_leaf_occurences(x, X_train, y_train, index_train, current_node_id = None): # set current_node = x.tree[0]
    ...
    if current_node_id in x.leafs:
        ...
        x.data_in_leaf[current_node_id] = index_train
    ...
    for current_node in x.tree:
        if current_node[0] == current_node_id:
            x.data_in_node[current_node_id]= index_train
            break
    ...
    if nr < mask.shape[0]: # some go left
        ...
        occ_left = improved_leaf_occurences(x, X_train[~mask], y_train[~mask], index_train[~mask], current_node[1])
        occ += occ_left # concatenate lists (of counters)
    if nr > 0: # some go right
        #print(" right child id = ", current_node[2])
        occ_right = improved_leaf_occurences(x, X_train[mask], y_train[mask], index_train[mask], current_node[2])
        occ += occ_right
    return(occ)
...
def node_occurences(x, X_train, expected = None, current_node_id = None):
    ...
    thresh = current_node[4]
    feat_num = current_node[3]
    mask = X_train[:,feat_num] > thresh #so those are de X_train in current_node
...
def calculate_leaf_occurences(x, X_test = None, y_test = None, pseudo_count = 1, categories = None, verbose = False, accuracy = False):
    ...
    index_train = np.array([i for i in range(len(x.X_train))])
    occ_dict = dict(improved_leaf_occurences(x, x.X_train, x.y_train,index_train))
