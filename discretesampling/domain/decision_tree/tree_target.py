...
class TreeTarget(DiscreteVariableTarget):
    ...
    def features_and_threshold_probabilities(self, x, continuous_thresholds = True, verbose = False):
        ...
        #
        logprobability = np.sum(logprobabilities) - math.log(catalan(len(x.tree)))# performance update# second term accounts for all the possible tree shapes (different once thresholds are placed in them) 
    ...
    def evaluateChipmanPrior(self, x):
        # From 'Bayesian CART model search' - Chipman et al. 1999
        def p_node(a, b, d):
